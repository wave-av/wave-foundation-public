...
| `.coderabbit.yaml` | ✅ | ✅ | — | Review automation config. Approval control lives in branch protection (human review via CODEOWNERS), not this file. |
...
applied via an **org-level ruleset** (PR required · required status checks · `enforce_admins` on ·
`required_approving_review_count >= 1`, satisfied by a human review from CODEOWNERS, not by any bot
auto-approval). That count >= 1 standard is what the weekly drift audit checks; `wave-foundation/master`
itself is grandfathered at count 0 (see `scaffolder/README.md`), where the required status checks are the
real merge gate.
