...
| 15 | **Advisor cascade** (catalysis) | **Near-advisor quality at ~executor cost** — a cheap EXECUTOR consults a stronger ADVISOR mid-generation in **one** call; the advisor catalyzes the plan but isn't consumed in the bulk tokens (billed separately as `usage.iterations[advisor_message]`) | `tools:[{type:"advisor_20260301",name:"advisor",model:<adv>,max_tokens:2048}]` + beta `advisor-tool-2026-03-01`; executor = top-level `model` (dispatch: routable key `exec+advisor:adv`, a crownable bandit arm — `WAVE_BANDIT_FRONTIER_ARMS`) | the executor isn't materially cheaper than the advisor (no savings); invalid pairs **400** (advisor must ≥ executor; Fable 5 / Mythos 5 are **self-pair-only**); `max_tokens` floors at 1,024 (rec 2,048, ~7× cheaper than the executor's output rate) |
...
- **Min cacheable prefix:** Fable 5 = **512** (1,024 on Bedrock); Opus 4.8 + Sonnet 4.6/4.5 = **1,024**; Opus 4.7 = **2,048**; Opus 4.6/4.5 + Haiku 4.5 = 4,096. Below it, caching silently no-ops — verify with `usage.cache_read_input_tokens`. (Full table + nuance in `model-matrix.md`.)
...
- **Advisor cascade (lever #15):** wired in the dispatch frontier (`wave_frontier`, routable `exec+advisor:adv`, #319) + chassis (#518), and crownable as a bandit arm (`WAVE_BANDIT_FRONTIER_ARMS`, #323) so the VoI/homeostat gate A/B's it vs the local/solo alternatives on live reward. Valid executor/advisor pairs are API-enforced.
