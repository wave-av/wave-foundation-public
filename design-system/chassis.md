...
Dark, terminal-native, **one accent**, mono everywhere — served **straight from the edge worker as
...
|---|---|---|
| **Tokens** | `tokens.css.ts` | `:root` palette — `--bg/--fg/--dim/--warn` fixed, **only `--acc` varies**; themed `::selection` |
| **Component CSS** | `chassis.css.ts` | ~30 lines that *are* the look — never edited per product |
...
The whole terminal-native look comes from a small, fixed set of classes — a spoke *composes* these,
never invents CSS:

- **Layout** — `body` (centered card), `.card`, `.top` (header row), `.foot` (footer)
- **Text** — `.sub` (muted lede), `.acc` / `.warn` / `.dim` (token colors), `h1` / `h2`, `code`
- **Blocks** — `pre` (ascii flow diagrams), `.box` (bordered panel), `.row`+`.k` (label:value lines)
- **Nav / CTA** — `.btn` (filled accent button), `.tag` (pill), `a` (accent links)
- **Data** — `.r`+`.kk` (panel rows), `.good` (positive/success, accent-colored)
...
- **Mono everywhere** — `ui-monospace, SFMono-Regular, Menlo, monospace`. Reinforces the
  developer/agent-tool identity. (Switch to a humanist sans only for a consumer-facing product.)
...
Extracted from `wave-dispatch-demo/edge-router/{worker,content,auth-portal}.ts` (the live dispatch
portal). The favicon path and the component CSS are lifted verbatim so a new spoke is visually
indistinguishable from dispatch except for its claimed accent.
